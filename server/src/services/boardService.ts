...
export default class BoardService implements IBoardService{
    ...
    public async createBoard(boardDTO: IBoardInputDTO): Promise<Result<{boardDTO: IBoardDTO}>>{
        ...
        const title = BoardTitle.create(boardDTO.boardTitle)
        if(title.isFailure){
            return Result.fail<{boardDTO: IBoardDTO}>("Title is invalid")
        }
        const board = Board.create({
            boardOwner: userFound,
            boardTitle: title.getValue(),
            boardPermissions: boardPermissions,
            fluidId: boardDTO.fluidId
        })

        if(board.isFailure){
            return Result.fail<{boardDTO: IBoardDTO}>(board.errorValue() )
        }
